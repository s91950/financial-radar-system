import { AreaChart, Area, ResponsiveContainer } from 'recharts'

const SIGNAL_COLORS = {
  positive: { dot: 'bg-green-400', ring: 'ring-green-400/20', glow: 'shadow-green-500/20' },
  neutral: { dot: 'bg-yellow-400', ring: 'ring-yellow-400/20', glow: 'shadow-yellow-500/20' },
  negative: { dot: 'bg-red-400', ring: 'ring-red-400/20', glow: 'shadow-red-500/20' },
}

export default function MarketIndicatorCard({ item, sparkData, isSelected, onClick, onSettingsClick }) {
  const signal = SIGNAL_COLORS[item.signal_status] || null
  const isUp = (item.change_percent || 0) >= 0

  return (
    <div
      onClick={onClick}
      className={`card-hover cursor-pointer relative group transition-all ${
        isSelected ? 'border-primary-500/50 ring-1 ring-primary-500/20' : ''
      } ${signal ? `shadow-md ${signal.glow}` : ''}`}
    >
      {/* Settings gear */}
      <button
        onClick={(e) => { e.stopPropagation(); onSettingsClick?.(item) }}
        className="absolute top-2 right-2 p-1 rounded opacity-0 group-hover:opacity-100 hover:bg-dark-600 transition-all text-dark-400 hover:text-white"
        title="信號設定"
      >
        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.324.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 011.37.49l1.296 2.247a1.125 1.125 0 01-.26 1.431l-1.003.827c-.293.24-.438.613-.431.992a6.759 6.759 0 010 .255c-.007.378.138.75.43.99l1.005.828c.424.35.534.954.26 1.43l-1.298 2.247a1.125 1.125 0 01-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.57 6.57 0 01-.22.128c-.331.183-.581.495-.644.869l-.213 1.28c-.09.543-.56.941-1.11.941h-2.594c-.55 0-1.02-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 01-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 01-1.369-.49l-1.297-2.247a1.125 1.125 0 01.26-1.431l1.004-.827c.292-.24.437-.613.43-.992a6.932 6.932 0 010-.255c.007-.378-.138-.75-.43-.99l-1.004-.828a1.125 1.125 0 01-.26-1.43l1.297-2.247a1.125 1.125 0 011.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.087.22-.128.332-.183.582-.495.644-.869l.214-1.281z" />
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
        </svg>
      </button>

      {/* Header: signal dot + name + symbol */}
      <div className="flex items-center gap-2 mb-2">
        {signal && (
          <span className={`w-2.5 h-2.5 rounded-full ${signal.dot} ring-4 ${signal.ring} flex-shrink-0`} />
        )}
        <span className="text-sm text-dark-300 truncate">{item.name}</span>
      </div>

      {/* Price + change */}
      <div className="flex items-baseline gap-2 mb-1">
        <span className="text-xl font-bold tabular-nums">
          {item.price != null ? item.price.toLocaleString(undefined, { maximumFractionDigits: 2 }) : '---'}
        </span>
        <span className={`text-sm font-medium ${isUp ? 'text-green-400' : 'text-red-400'}`}>
          {isUp ? '+' : ''}{(item.change_percent || 0).toFixed(2)}%
        </span>
      </div>

      {/* Sparkline */}
      {sparkData && sparkData.length > 1 && (
        <div className="h-10 -mx-1 mt-1">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={sparkData}>
              <defs>
                <linearGradient id={`spark-${item.symbol}`} x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor={isUp ? '#4ade80' : '#f87171'} stopOpacity={0.3} />
                  <stop offset="95%" stopColor={isUp ? '#4ade80' : '#f87171'} stopOpacity={0} />
                </linearGradient>
              </defs>
              <Area
                type="monotone"
                dataKey="close"
                stroke={isUp ? '#4ade80' : '#f87171'}
                fill={`url(#spark-${item.symbol})`}
                strokeWidth={1.5}
                dot={false}
                isAnimationActive={false}
              />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      )}

      {/* Symbol label */}
      <div className="text-xs text-dark-500 mt-1">{item.symbol}</div>
    </div>
  )
}

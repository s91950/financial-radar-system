...
async def test_rss_source(source_id: int, db: Session = Depends(get_db)):
    ...
    if source.type == "website":
        ...
        from backend.services.businessweekly_scraper import is_businessweekly_url, fetch_businessweekly_news
        from backend.services.yahoo_scraper import is_yahoo_url, fetch_yahoo_news
        ...
        scraper_map = [
            ...
            (is_businessweekly_url, lambda: fetch_businessweekly_news(hours_back=24), "商周今日最新"),
            (is_yahoo_url, lambda: fetch_yahoo_news(source.url, hours_back=24), "Yahoo Finance Ticker Feeds"),
        ]

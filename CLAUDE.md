...
- **第二階段（圖表 UI）**：新元件 [MarketChartPanel.jsx](frontend/src/components/Radar/MarketChartPanel.jsx) 取代 DashboardPage 內嵌的舊走勢圖，提供日期範圍（5天/1月/3月/6月/1年/2年）、「+ 比較」多指標疊圖（上限 5 條）、右上角資料來源連結 icon。
  - **絕不開雙軸**（dataviz 規範的頭號禁忌：兩個 y 軸的對齊方式是任意的，會憑空造出不存在的相關性）。跨類別／不同尺度比較改用兩個維度的控制：
    - **檢視方式**：`疊圖`（全部畫在同一張）/ `分面`（小倍數 —— 每個指標各自一張小圖、**保留原始單位與刻度**，時間軸靠 recharts `syncId` 對齊，滑鼠移到任一張其餘圖的游標會同步）。要讀實際數值就用分面。
    - **疊圖的縮放模式**：`原始值`（單位相同時可用，單位不同會自動 disable）/ `指數化`（÷起始值×100，看相對漲跌幅）/ `標準化`（z-score，看不同尺度是否同向）。單位不同時預設自動切到指數化，但**使用者手動選過就尊重他的選擇**（`userPickedScale` flag）。
    - 指數化對殖利率會失真（4.00→4.50 是 +12.5%，但利率的語言是 +50bp），圖上有文字說明並引導到分面模式。
...
- **踩坑（上線後才發現）**：(1) `_history_batch_sync` 原本用**代碼數量**判斷 yfinance 的欄位結構（`df[sym] if len(symbols) > 1 else df`），但 `yf.download(group_by="ticker")` **即使只給一個代碼也回 MultiIndex 欄位** `('^TNX','Close')` —— 單一代碼時取 `df["Close"]` 會 KeyError，又被裸 `except` 吞掉變成靜默回空。症狀是**點任一張卡片開出來的走勢圖都是空的**，但走勢縮圖（一次 34 檔）與利差（連同兩條腿一起抓）反而正常。修法：一律檢查 `isinstance(df.columns, pd.MultiIndex)` 並依 ticker 層索引，**不要用數量判斷**；例外改成有記錄的 warning。(2) recharts 的 `dataKey` 是字串時會被當成 lodash 巢狀路徑，`DX-Y.NYB` 會去找 `row['DX-Y']['NYB']` —— 圖表列資料改用安全鍵 `s0/s1/…`，顯示時再映射回代碼。
- **利差可由使用者自訂**（admin+）：儀表板「市場指標總覽」右上角「+ 新增利差」→ [AddDerivedModal.jsx](frontend/src/components/Radar/AddDerivedModal.jsx)。簡易模式是「指標 A − 指標 B」兩個下拉（自動帶入名稱／代碼／分類），進階模式可直接寫算式。**存檔前會即時試算**（`POST /api/radar/market/derived/preview`，debounce 500ms），看得到當前值與各成分的值。後端 `_validate_formula()` 擋四件事：沒引用任何指標、引用不存在的指標、**引用其他衍生指標**（避免相依環）、代換後算不出數字。單位在成分一致時自動繼承（殖利率相減 → `percent` → 變化自動以 bp 顯示）。
- **刪除保護**：`DELETE /market/watchlist/{id}` 會先檢查該指標是否還被某個利差的算式引用，是的話擋下並列出是哪些 —— 直接刪掉成分會讓那個利差從此算不出值，而錯誤只出現在後端日誌，使用者只看到一張空卡片。刪除入口在指標卡的齒輪 → 信號條件視窗左下角「刪除此指標」。
- **資料新鮮度：改用德國央行取代 ECB 當歐洲主力**。實測各來源最新資料日（2026-09-15 週二量測）：Yahoo 美債 T-0 即時、**日本財務省 T-1**、**德國央行 T-1**、ECB **T-3**（落後 2 個交易日）、BoE **T-5**（落後 3 個交易日；`IUDMNZC` / `IUDSNPY` / `IUDLNPY` 三支都一樣，是來源本身慢，換 series code 無效；UK DMO 被 ShieldSquare captcha 擋）。新增 `bundesbank` provider（BBSIS，tenor 代碼 `R02XX`/`R05XX`/`R10XX`/`R30XX`，非交易日的值是 `.`），建立德 2Y/10Y/30Y 指標，並把**美歐利差改為美德利差**（`{^TNX} - {DE10Y}`）—— ECB 聚合落後兩個交易日會讓利差跟著失真。ECB 那組保留為歐元區整體參考。
- **衍生指標的資料時點取「最舊的成分」**：美德利差混了即時的美債與 T-1 的德債，標成即時會讓人以為兩腿同一天。當期值仍用各腿最新可得值（與行情終端一致），歷史序列則只取兩腿都有值的日期。
...
- **主題頁 UI 收斂（同日稍後）**：(1) 移除展開後 header 的關鍵字顯示區塊 —— 佔掉太多垂直空間、擠壓下方清單，關鍵字改只在編輯彈窗看；(2) **GN 搜尋功能整個移除**（前端「搜尋並匯入」按鈕 + 時間範圍選擇器、後端 `POST /topics/{id}/search` 與 `_multi_query_search` / `_parse_keyword_groups` / `_build_topic_gn_query` / `SearchImportRequest` / `_parse_dt` 全刪、`topicsAPI.searchAndImport` 也刪）—— 主題既然是分類層就不該有自己的抓取入口；(3) 每則新聞與市場警示尾端顯示命中關鍵字 badge，樣式與規則比照 RadarPage（`extractMatchedKw`：按分隔符切前 4 個 term、超過 40 字截斷），資料來自 `TopicArticle.matched_keyword` / `TopicSignal.matched_keyword`。**連帶清除**：`GroupedKeywordCard` 的「風險標記」popover —— 唯一傳 `severityKws` / `onAddToSeverity` 的呼叫點是被移除的 header 區塊，留著只會開一個什麼都不做的浮層。**同日再收斂**：來源篩選 pills（全部來源／雷達／手動）連同 `filterSource` state 一併移除——GN 搜尋拿掉後不會再有 `add_source='manual'` 的新資料，這組篩選失去意義；統計列的「雷達自動匯入／手動搜尋」兩個計數同時拿掉（它們就是這組篩選的來源拆分），只留「共 N 篇 ∣ 市場警示 N」。篩選列因此壓回**兩列**（統計+批次操作 / 內容類型+風險+搜尋+排序），移除 `flex-1` spacer 讓元素靠左packing。批次操作加「刪除」：`handleDeleteSelected()` 對選取的文章分批 5 筆並行呼叫 `DELETE /topics/{id}/articles/{aid}`（後端無批次端點，分批是避免對 e2-micro 的 SQLite 一次開幾十個並行交易），個別失敗不中斷整批、最後回報成功與失敗數。
- **連帶修掉 `article_fetcher` 的「長度即品質」缺陷**（[article_fetcher.py](backend/services/article_fetcher.py)）：`enrich_articles_with_full_body()` 原本只要抽到的文字比現有 content 長就覆蓋。**太報（taisounds）本文是 JS 渲染的**，靜態 HTML 只有站內「快訊」跑馬燈，抽出來 2700 字全是別篇報導的標題，硬是蓋掉 scraper 存的正確 `og:description`。近 7 天 38 篇太報文章有 14 篇中招（其他來源 0 篇），連帶污染排除關鍵字過濾、嚴重度評估與 `Article.content`。修法：`_extract_main_text()` 對**最終選中的節點**（不分 `<article>`/`<main>`/最大 `<div>` 哪條路徑）檢查 `_has_real_paragraph()` —— 至少一個 ≥40 字、非版權/Cookie 樣板的 `<p>`；沒有就回 `None`，保留 scraper 原內容不覆蓋。實測 7 個主要來源只有太報被擋下，含同樣走 `largest-div` fallback 的自由財經在內其餘全部行為不變。
- **未做**：`cross_above` / `cross_below`（註解有寫但仍未實作）。
...
2. **主題追蹤 (Topics)** `/search` — 純**分類層**：主題自己不抓新聞（**無任何 GN 搜尋入口**），而是把雷達以「全域關鍵字 + 來源關鍵字」抓進來的文章，用主題關鍵字歸類（詳見上方 2026-09-15 changelog）。除新聞外也收**市場數據警示**（`TopicSignal`）——`market_check` 觸發時依「關鍵字自動比對」或 `Topic.bound_symbols` 手動綁定歸入。唯一的手動動作是「回溯比對」（拿既有 `Article` / market `Alert` 重跑比對）。每則新聞與警示尾端顯示命中關鍵字 badge。
...
| `/api/topics` | `routers/topics.py` | Topic CRUD（含 `bound_symbols`）、per-topic articles **+ signals**（`GET /{id}/articles` 同時回 `articles` / `signals` / `stats`）、`DELETE /{id}/articles/{aid}`、`DELETE /{id}/signals/{sid}`、`POST /{id}/rematch`（回溯比對既有 `Article` 與 market `Alert`，body `{days, include_signals}`，days 上限 90）、`GET /market-symbols`（可綁定的指標清單，給主題編輯頁勾選）。**已移除** `POST /{id}/search`（GN 搜尋匯入）—— 主題是分類層，不該有自己的抓取入口。**路由順序**：`GET /market-symbols` 與 `/{topic_id}/articles` 路徑段數不同，不衝突。 |
